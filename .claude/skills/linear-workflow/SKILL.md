...
description: Work on Linear issues with enforced status tracking. Use when picking up, working on, or completing Linear issues. Triggers on requests to work on issues, fix review items, or process Linear backlog.
---
...
1. Pick the most urgent unassigned Linear issue in your assigned area label
2. **Immediately** set status to `In Progress` and assign to yourself
...
1. Enter plan mode
2. Read the issue description, referenced review files (`.claude/review.md`, `.claude/best-practices-audit.md`)
...
- Only set status to `Canceled` (with a comment explaining why) after the user explicitly approves.
- **Stop here** — do not proceed to implementation

If the issue is a symptom of something bigger:

- Comment the root cause on the issue
...
7. Commit with the Linear issue identifier in the message (e.g. `I-123: Fix rate limiting on login endpoint`). One commit per issue, don't push or create PRs.
...
- If the issue was invalid → set status to `Canceled` (only after user approved cancellation in Phase 2)

This step is non-negotiable. Never finish work on an issue without updating its status. If the Linear API call fails, retry once, then inform the user that the status needs manual update.
...
- [ ] Committed with Linear issue ID in the message (e.g. `I-123: ...`)
- [ ] Status set to `Done` or `Canceled` AFTER all work is complete
