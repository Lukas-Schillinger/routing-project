import { routeService, routeShareService, ServiceError } from '$lib/services/server';
...
export const load: PageServerLoad = async ({ params, url }) => {
	try {
		...
		if (publicRoute) {
			return await routeService.getRouteWithDetails(params.routeId, publicRoute.organization_id);
		}

		// 2. Check for valid share token in URL
		const token = url.searchParams.get('token');
		if (token) {
			const routeWithDetails = await routeShareService.validateTokenAndGetRoute(token);
			if (routeWithDetails) {
